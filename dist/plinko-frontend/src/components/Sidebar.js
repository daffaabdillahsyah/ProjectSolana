...
function Sidebar({ balance, houseProfit, serverSeedHash, lastResult, betHistory }) {
    ...
    return (<div className="w-80 space-y-6">
      ...
      {lastResult && (<div className="bg-gray-900 p-6 rounded-2xl shadow-lg">
      ...
      {betHistory.length > 0 && (<div className="bg-gray-900 p-6 rounded-2xl shadow-lg">
          <h3 className="text-white text-lg font-semibold mb-4">Recent History</h3>
          <div className="space-y-2">
            {betHistory.slice(-5).reverse().map((bet) => (<div key={bet.betId} className="flex justify-between items-center py-2 border-b border-gray-800 last:border-b-0">
