...
export default function Controls({
  ...
  return (
    <div className="w-80 bg-gray-900 p-6 rounded-2xl shadow-lg space-y-6">
      ...
      <div className="space-y-3">
        ...
        <div className="flex space-x-2">
          <button
            onClick={halveBet}
            disabled={disabled}
            className="flex-1 bg-gray-800 hover:bg-gray-700 text-white py-2 px-4 rounded-xl border border-gray-700 disabled:opacity-50 transition-colors"
          >
            ÷2
          </button>
          <button
      ...
      <div className="space-y-3">
        ...
        <div className="flex space-x-2">
          ...
          <button
            ...
            disabled={disabled}
            className="bg-gray-800 hover:bg-gray-700 text-white px-4 py-3 rounded-xl border border-gray-700 disabled:opacity-50 transition-colors"
          >
            🎲
          </button>
        </div>
      ...
      {/* Fixed Settings Caption */}
      <div className="text-gray-400 text-xs text-center">
        Fixed: rows = {FIXED_ROWS}, risk = {FIXED_RISK}
      </div>
    </div>
  );
}
